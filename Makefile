...
build:
	...
	go build -o geth -ldflags="\
    	-X 'github.com/vishnushankarsg/geth/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/vishnushankarsg/geth/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/vishnushankarsg/geth/versioning.Branch=$(BRANCH)'\
		-X 'github.com/vishnushankarsg/geth/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/geth .
	env EDGE_BINARY=${PWD}/artifacts/geth go test -v -timeout=30m ./e2e/...
